...
st.title("🛡️ Institutional Risk & Strategy Terminal")
st.write(
    "Multi-asset quantitative engine featuring technical crossovers, tail-risk metrics (VaR/CVaR), and portfolio correlation analysis."
)
...
with tab1:
    ...
    ticker = st.sidebar.text_input("Enter Stock Ticker", value="RELIANCE.NS").strip()
    benchmark_ticker = st.sidebar.text_input("Benchmark Index Ticker", value="^NSEI").strip()
    ...
    with st.spinner("Downloading financial data..."):
        df = get_data(ticker_clean, time_period)
    ...
    if df.empty or len(df) <= slow_ma:
        ...
        csv_data = df_clean[["Close", "Fast_MA", "Slow_MA", "Strategy_Return", "Strategy_Equity"]].to_csv()
        st.download_button(
...
with tab2:
    ...
    user_tickers = st.text_input("Enter Stock Tickers (comma-separated)", value=default_tickers)
